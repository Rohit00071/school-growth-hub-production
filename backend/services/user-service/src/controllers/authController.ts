...
const DEFAULT_SECRET = JWT_SECRET || 'your-super-secret-key-change-this-in-production';
logger.info(`[Auth Debug] Active JWT_SECRET: ${DEFAULT_SECRET.substring(0, 5)}...`);
...
export const login = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        logger.info(`[Login Debug] Attempting login for: ${validatedData.email}`);

        if (!user) {
            logger.warn(`[Login Debug] User not found: ${validatedData.email}`);
            return res.status(401).json({ status: 'fail', message: 'User not found' });
        }

        const isPasswordValid = await bcrypt.compare(validatedData.password, user.password);
        logger.info(`[Login Debug] Password valid: ${isPasswordValid} for user ${user.id}`);
